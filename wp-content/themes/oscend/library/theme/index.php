<?php
	...
	require_once( get_template_directory() . '/library/theme/import.php');



	add_action('after_setup_theme', 'clinic_theme_support_setup');
	function clinic_theme_support_setup(){
		add_theme_support('clinic_customize_opt');
		add_theme_support('default_customize_opt');
		add_theme_support( 'wc-product-gallery-zoom' );
		add_theme_support( 'wc-product-gallery-lightbox' );
		add_theme_support( 'wc-product-gallery-slider' );
		add_image_size('clinic-thumb', 70, 70, false);
        }


	// Plugin Yellow Pencil: Visual CSS Style Editor
	add_site_option( 'YP_PART_OF_THEME', 'true' );
	if ( oscend_get_option( 'general_settings_css_live_editor', 'off' ) == 'on' ) {
