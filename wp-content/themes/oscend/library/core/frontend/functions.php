<?php
	...
	function oscend_js_vars(){

		$vars = '';

		$_js = apply_filters('oscend_js_vars',$vars);
	...
	function oscend_css_vars(){

		$css = '';
		$header_color  = oscend_get_option('header_all_color');
		$footer_color  = oscend_get_option('footer_all_color');
		if ($footer_color){
	...
	function oscend_get_theme_header(){
		$headerType = 1;
		global $wp_query;
	...
	function oscend_load_block($block_name){
	...
	function oscend_woo_get_page_id(){
	...
	function oscend_checkAvailableJsToPage($types){
	...
	function oscend_get_staticblock_content($blockId) {
			...
			echo do_shortcode(apply_filters('the_content', $block->post_content));
		
	}


	function oscend_get_staticblock_option_array(){
		...
		$staticBlocks = array();
		$staticBlocks['default'] = esc_html__('Select block', 'oscend');
