<?php
	...
	function oscend_sanitize_text( $input ) {
		return wp_kses_post( $input );
	}

	function oscend_sanitize_checkbox( $input ) {
		if ( $input == 'on' ) {
			return 'on';
		} else {
			return '';
	...
	function oscend_sanitize_per_page( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function oscend_sanitize_absinteger( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function oscend_sanitize_email( $email ) {
		if (is_email( $email ) ) {
			return $email;
		} else {
			return '';
	...
	function oscend_sanitize_layout( $value ) {
	...
	function oscend_sanitize_loader( $value ) {
	...
	function oscend_sanitize_onoff( $value ) {
	...
	function oscend_sanitize_sidebar_blog_type( $value ) {
	...
	function oscend_sanitize_sidebar_blog_content( $value ) {
	...
	function oscend_sanitize_header_type( $value ) {
	...
	function oscend_sanitize_overlay_opacity( $value ) {
	...
	function oscend_sanitize_portfolio_type( $value ) {
	...
	function oscend_sanitize_portfolio_perrow( $value ) {
	...
	function oscend_sanitize_sidebar_portfolio_type( $value ) {
		if ( ! in_array( $value, array( '1', '2', '3' ) ) ) {
			$value = '2';
		}
	...
	function oscend_sanitize_sidebar_portfolio_content( $value ) {
	...
	function oscend_sanitize_animation( $value ) {
	...
	function oscend_sanitize_footer_block($input) {
		$valid = oscend_get_staticblock_option_array();
